...
Function Get-PrivateProfileSection {
    ...
    $parameterTypes = [string], [System.Text.StringBuilder], [int], [string]
    $parameters = [string] $category, [System.Text.StringBuilder] $returnValue, [int] $returnValue.Capacity, [string] $file

    ## Invoke the API
    [void] (Invoke-WindowsApi "kernel32.dll" ([UInt32]) "GetPrivateProfileSection"   $parameterTypes $parameters)
...
#region Metadata
    ...
    $FuncName        = 'Get-PrivateProfileSection'
    $FuncAlias       = ''
